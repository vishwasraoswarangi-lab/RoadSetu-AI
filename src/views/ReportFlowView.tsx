...
export const ReportFlowView: React.FC<ReportFlowViewProps> = ({
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      if (result.status === 'success' && result.coords) {
        try {
          const detected = await reverseGeocodeCoords(
          ...
          if (active) {
            setLocation({
              ...
              formattedAddress: result.city || 'Current location',
        ...
        setLocationError(
          result.errorMessage || 'Location access was not available.'
        );
      }
  ...
  const openCamera = async () => {
    if (!navigator.mediaDevices?.getUserMedia) {
      showToast(
        'Live camera is not supported here. Use Upload Image instead.',
        'info'
      );
  ...
  const captureFrame = () => {
    ...
    showToast(
      'Image captured. Review it before using it.',
      'success'
    );
  };
  ...
  const refreshLocation = async () => {
    ...
    if (result.status === 'success' && result.coords) {
      ...
      setLocationError(
        result.errorMessage || 'Unable to retrieve your location.'
      );
    }
  ...
  const analyzeAndSubmit = async () => {
    if (!photoUrl || !base64Image) {
      showToast(
        'Capture or upload a road image first.',
        'info'
      );
      return;
    }
    ...
    if (!location.latitude || !location.longitude) {
      showToast(
        'Add a valid location before submitting.',
        'info'
      );
      return;
    }
    ...
    const nearby = checkForDuplicates(
      location.latitude,
      location.longitude
    );
    ...
    try {
      ...
      if (!data?.defectType || !data?.severity) {
        throw new Error(
          'AI analysis returned an incomplete result.'
        );
      }
      ...
      const created = await addComplaint({
        description:
          description.trim() ||
          result.aiSummary ||
          'Road defect reported from image analysis.',
        location,
      ...
      showToast(
        error instanceof Error
          ? error.message
          : 'Could not complete the report. Please try again.',
        'error'
      );
    } finally {
      setAnalyzing(false);
      ...
      <div className="min-h-[80vh] bg-slate-50 px-4 py-10 flex items-center justify-center">
        <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/60">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Sign in to submit a verified civic report with
            your image and location.
          </p>

          <button
            onClick={() => openAuthModal('login')}
            className="mt-7 w-full rounded-xl bg-blue-600 px-5 py-3.5 text-sm font-bold text-white transition hover:bg-blue-700"
          >
            Sign in to continue
          </button>
        </div>
      ...
      <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-4 flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                Capture the road condition, confirm where it is,
                and let AI analyze the evidence.
              </p>
            </div>

            <div className="hidden items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-600 shadow-sm sm:flex">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Use a fresh photo or choose one from your device.
                </p>
              </div>
            ...
            {photoUrl ? (
              <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-100">
                ...
                <button
                  ...
                  className="absolute right-3 top-3 rounded-full border border-white/70 bg-white/90 p-2 text-slate-700 shadow-md backdrop-blur hover:bg-white"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="flex min-h-[360px] flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-6 text-center">
                ...
                <h3 className="font-bold text-slate-900">
                  Add a clear road image
                </h3>

                <p className="mt-1 max-w-sm text-xs leading-5 text-slate-500">
                  A visible road surface helps the AI identify
            ...
            <label className="mt-6 block">
              ...
              <textarea
                ...
                rows={4}
                placeholder="e.g. Large pothole near the left lane after the junction..."
                className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-400 focus:bg-white focus:ring-4 focus:ring-blue-50"
              />
            </label>
          </section>
          ...
          <aside className="space-y-5">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-500">
                    Used to route the report to the right area.
                  </p>
                </div>
              ...
              <button
                ...
                className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
              >
            ...
            <section className="rounded-3xl border border-blue-100 bg-blue-50/60 p-5">
              ...
              {analysis ? (
                <div className="mt-5 space-y-3">
                  ...
                  <p className="rounded-xl border border-blue-100 bg-white p-3 text-xs leading-5 text-slate-600">
                    {analysis.aiSummary ||
                      'Analysis completed.'}
                  </p>
                ...
                <div className="mt-4 rounded-2xl border border-blue-100 bg-white p-4">
                  <p className="text-xs leading-5 text-slate-500">
                    Your image is analyzed when you submit.
                    The result will appear here after the AI
                    review is complete.
                  </p>
                </div>
              )}
        ...
        {nearbyReports !== null && (
          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            ...
            Another citizen may have reported an issue close
            to this location. This is only a warning — it does
            not prevent you from submitting a separate report.
          </div>
        )}
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-7 py-3 text-sm font-black text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {analyzing ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
        ...
        <div className="mt-4 flex items-center justify-center gap-2 text-[11px] text-slate-400">
          ...
          Your authenticated account is attached to the report.
        </div>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  {cameraPreview
                    ? 'Review the photo before adding it to your report.'
                    : 'Camera access is used only while this capture window is open.'}
                </p>
              </div>
            ...
            <div className="grid gap-3 p-5 sm:grid-cols-2">
              ...
              {cameraPreview ? (
                <>
                  <button
                    onClick={retakePhoto}
                    className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-black text-white transition hover:bg-blue-700"
                  >
                    <CheckCircle2 className="h-4 w-4" />
                    Use Photo
                  </button>
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 text-sm font-black text-white transition hover:bg-blue-700 sm:col-span-2"
                >
                  <Camera className="h-4 w-4" />
                  Capture Image
                </button>
